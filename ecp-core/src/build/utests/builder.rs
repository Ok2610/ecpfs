...
fn resolve_dtype_uses_requested_when_set_else_native() {
    assert_eq!(resolve_dtype(None, EmbeddingDtype::F32), EmbeddingDtype::F32);
    assert_eq!(resolve_dtype(None, EmbeddingDtype::F16), EmbeddingDtype::F16);
    assert_eq!(resolve_dtype(Some(EmbeddingDtype::F16), EmbeddingDtype::F16), EmbeddingDtype::F16);
    assert_eq!(resolve_dtype(Some(EmbeddingDtype::F32), EmbeddingDtype::F32), EmbeddingDtype::F32);
    assert_eq!(
...
fn select_representatives_uses_the_builders_max_chunk_bytes() {
    ...
    let mut builder =
        Builder::new(as_readable_writable_listable(&store), 2, Metric::L2, false, 1_000_000, None, 16);

    builder.select_representatives(&source, 3, RepresentativeStrategy::Offset, 10);

    let rep_embeddings =
        Array::open(as_readable_writable_listable(&store), "/rep_embeddings").expect("failed to open rep_embeddings");
    assert_eq!(rep_embeddings.chunk_shape_usize(&[0, 0]).expect("failed to read chunk shape"), vec![2, 2]);
}
...
fn build_with_custom_representatives_writes_index_root_and_leaf_nodes() {
    ...
    builder.select_representatives_custom(Array1::from_vec(vec![7u32, 9u32]), array![[0.0f32, 0.0], [10.0, 0.0]]);
    builder.build(&dataset, 10);
