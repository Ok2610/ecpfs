...
fn node_cache_is_unbounded_until_set_limit_is_called() {
    let store = new_memory_store();
    let store = as_readable_writable_listable(&store);
    append_node_batch(&store, "/lvl_1/node_0", "node_ids", &array![[1.0f32, 2.0]], &array![10u32], &[100, 2], EmbeddingDtype::F32);

    let cache = NodeCache::new();
    let first = cache.get_or_read(&store, "/lvl_1/node_0");
    let second = cache.get_or_read(&store, "/lvl_1/node_0");

    assert!(Arc::ptr_eq(&first, &second), "an entry must survive without set_limit ever being called");
}
...
fn node_cache_set_limit_evicts_the_least_recently_used_entry() {
    ...
    let store = as_readable_writable_listable(&store);
    append_node_batch(&store, "/lvl_1/node_0", "node_ids", &array![[1.0f32, 2.0]], &array![10u32], &[100, 2], EmbeddingDtype::F32);
    append_node_batch(&store, "/lvl_1/node_1", "node_ids", &array![[3.0f32, 4.0]], &array![20u32], &[100, 2], EmbeddingDtype::F32);
    ...
    assert!(Arc::ptr_eq(&node_0_first, &node_0_after), "node_0 was touched more recently, so it must survive eviction");

    let node_1_after = cache.get_or_read(&store, "/lvl_1/node_1");
    assert!(!Arc::ptr_eq(&node_1_first, &node_1_after), "node_1 was the least-recently-used entry, so it must have been evicted");
}
...
fn node_cache_get_or_read_is_safe_under_concurrent_access_to_the_same_path() {
    ...
    assert!(Arc::ptr_eq(first, &after), "byte accounting must not be inflated by concurrent misses racing on the same entry");
}
